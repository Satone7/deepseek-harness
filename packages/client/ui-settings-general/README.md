...
The General section holds rows registered into `settings.general.item` by feature packages — its only built-in row is the read-only version row, and even that appears only when the Web host injected the serving release. Feature plugins own the row copy and behavior; the shell only provides the section and its slot. The Appearance row, for example, lives in ui-theme.
...
- **The General section's only built-in row is the version row** — every other row appears only when its owning feature plugin is mounted.
