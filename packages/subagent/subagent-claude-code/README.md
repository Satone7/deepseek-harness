...
In this repository's `dsh-base`, the provider is installed and mounted once on the host plane, and the shipped full Agent Presets enable the matching `subagent_claude_code` tool row. Loading the provider starts no Claude process until a tool call. If you are composing a custom Profile from a base that does not include it, install `@deepseek-ai/dsh-subagent-claude-code` and mount it once on the host plane; in a custom preset, add the tool row without `disabled: true` to expose `subagent_claude_code` only to agents composed from that preset. Its `one-shot` policy keeps omitted or `false` `run_in_background` calls in the foreground, while explicit `true` returns a parent-owned Job id for `job_output` or `job_kill`. The base host and full presets already provide the generic Job registry and controls.
