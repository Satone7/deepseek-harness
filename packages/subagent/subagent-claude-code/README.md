...
In this repository's `dsh-base`, the provider is installed and mounted once on the host plane, and the shipped full Agent Presets enable the matching `subagent_claude_code` tool row. Loading the provider starts no Claude process until a tool call. If you are composing a custom Profile from a base that does not include it, install `@deepseek-ai/dsh-subagent-claude-code` as a Bundle (below) and the tool row in a custom preset without `disabled: true` to expose `subagent_claude_code` only to agents composed from that preset. Its `one-shot` policy keeps omitted or `false` `run_in_background` calls in the foreground, while explicit `true` returns a parent-owned Job id for `job_output` or `job_kill`. The base host and full presets already provide the generic Job registry and controls.

This package is also an installable Profile Bundle. Install it into a target Profile, then restart that Profile; installation brings the pinned Agent SDK and one compatible platform CLI payload into that Profile, while the declared `cordis.patch.yml` layer registers only the dormant `claude-code` Host provider and starts no Claude process. Removing the package withdraws that provider and its private runtime closure on the next Profile start.
...
Installation controls Host availability, not model permission. The Bundle supplies the dormant default `claude-code` row; the Profile may replace that row's complete config or mount additional rows with distinct `providerName`, `permissionMode`, and `env` values. Loading an instance starts no Claude process until a bound tool calls it. Each `dsh-tool-subagent` row names one provider and needs its own `toolName`, so the model sees static tools rather than a dynamic provider selector. The shipped full Agent Presets enable the `subagent_claude_code` row; a copied preset can remove that row to withhold the tool from agents composed from the copy. Its `one-shot` policy keeps omitted or `false` `run_in_background` calls in the foreground, while explicit `true` returns a parent-owned Job id for `job_output` or `job_kill`. The base host and full presets already provide the generic Job registry and controls.
