...
Mount this provider when a delegation should run as a real Claude Code session in the parent's workspace. In this repository's `dsh-base` the provider is installed and mounted once on the host plane, and the shipped full Agent Presets enable the matching `subagent_claude_code` tool row, so base-backed profiles need no extra step. A custom Profile composed from a base without it follows the explicit path: install the Bundle into a Profile, optionally configure the provider row, and expose it to the model through a delegation tool row.
...
Each delegation tool row names one provider and needs its own `toolName`, so the model sees static tools rather than a dynamic provider selector. The shipped full Agent Presets enable the `subagent_claude_code` row; a copied preset can remove that row to withhold the tool from agents composed from the copy.
