 ...
 * settings page's configurable-plugins tab. Mounting problems are logged,
...
// Type-only: pulls the ctx.slots Context merge (ui-renderer owns the service).
import type {} from '@deepseek-ai/dsh-client-ui-renderer/client'
// Type-only: pulls the conversation slot declarations (composer.dock).
...
declare module '@deepseek-ai/dsh-client-ui-slots' {
  ...
  interface SlotMap {
    /**
     * The card seat the configurable-plugins tab (ui-settings-plugins)
     * declares and dispatches by settings namespace; this card registers
     * under the `token-cost` namespace key so the tab pairs it with the
     * host-registered settings section.
     */
    'settings.plugin.item': { kind: 'keyed'; scope: 'root'; owner: TokenCostSettingsCardFace }
  }
}
...
export function apply(ctx: ClientContext): void {
  ...
  try {
    ...
    disposers.push(ctx.slots.inject('settings.plugin.item', () => ctx.slots.register({
      ...
      // Keyed slot: the key is the identity (and the namespace the tab
      // dispatches on); keyed registers take no id/order.
      key: TOKEN_COST_NS,
      locale: NS,
