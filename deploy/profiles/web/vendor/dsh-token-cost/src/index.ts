...
import { parseTrustedNetwork, type TrustedNetwork } from './trust-fence.ts'

/**
 * The web runtime's bind-derived trust surface (the same source the official
 * /api gateway fence derives its list from): boot-sampled LAN literals plus
 * `--trusted-host` authorities, and declared `--trusted-network` CIDRs.
 */
...
declare module '@deepseek-ai/cordis' {
  interface Context {
    /** Live web runtime trust values, provided by the web-app bundle. */
    webRuntime?: WebRuntimeTrust
  }
}
...
export function apply(ctx: Context, config: Config = {}): void {
  ...
  const rebuild = (): void => {
    ...
    // Trust facts per request from the live webRuntime value; the CIDR
    // entries are parsed once (the official gateway already validated them
    // at load, so a parse failure here is a disagreement worth surfacing).
    let parsedNetworks: readonly TrustedNetwork[] | undefined
    const trust = (): { trustedHosts: readonly string[]; trustedNetworks: readonly TrustedNetwork[] } => {
      const runtime = ctx.webRuntime
      if (parsedNetworks === undefined) {
        parsedNetworks = (runtime?.trustedNetworks ?? []).map(parseTrustedNetwork)
      }
      return { trustedHosts: runtime?.trustedHosts ?? [], trustedNetworks: parsedNetworks }
    }
    disposeRoutes = ctx.effect(
      () => {
        const routes = makeRoutes({ ledger, pricing, schemes, customPrices, priceStore, trust })
