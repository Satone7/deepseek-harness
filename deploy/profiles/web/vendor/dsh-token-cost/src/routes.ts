 ...
 * listing, per-session detail and resync. Every route passes the same
 * browser-trust fence as the official /api gateway — Host-header loopback or
 * a deployment `trustedHosts` authority, browser same-origin markers, and
 * (when declared) a `trustedNetworks` socket-source gate — so LAN-exposed
 * deployments serve these facts exactly as they serve the /api surface,
 * while cross-site pages and rebound hosts stay refused.
 */
...
export interface TokenCostRoutesDeps {
  ...
  /** Trust facts read per request from the live webRuntime service value. */
  trust: () => { trustedHosts: readonly string[]; trustedNetworks: readonly TrustedNetwork[] }
}
...
/** Composed trust fence: socket-source gate plus the Host-header fence. */
function isTrustedRequest(
  request: IncomingMessage,
  trust: { trustedHosts: readonly string[]; trustedNetworks: readonly TrustedNetwork[] },
): boolean {
  return acceptsTrustedSource(request.socket.remoteAddress, trust.trustedNetworks)
    && isTrustedApiRequest(request, trust.trustedHosts)
}
...
export function makeRoutes(deps: TokenCostRoutesDeps): WebRoute[] {
  ...
  const guard = (req: IncomingMessage, res: ServerResponse, method: string): boolean => {
    if (!isTrustedRequest(req, deps.trust())) {
