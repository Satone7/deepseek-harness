...
| Profile | Arguments |
|---|---|
| `web` | `--host`, `--port`, repeatable `--trusted-host` and `--trusted-network` |
...
The production Web runner needs built package and frontend artifacts (`pnpm run build`). It serves `http://127.0.0.1:3080` by default. `--host 0.0.0.0` requires at least one `--trusted-network` and exits with a usage error without one; `--trusted-host` adds named authorities accepted by the `/api` browser-trust fence, and `--trusted-network` adds subnets whose socket sources that fence trusts like loopback.
